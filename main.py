...
flags.DEFINE_string('run_group', 'deflow', 'Run group.')
...
config_flags.DEFINE_config_file('agent', 'agents/deflow.py', lock_config=False)
...
def main(_):
    ...
    terminals = train_dataset.get('terminals', np.zeros_like(train_dataset['rewards']))
    timeouts = train_dataset.get('timeouts', np.zeros_like(train_dataset['rewards']))
    ...
    r_min, r_max = np.min(train_dataset['rewards']), np.max(train_dataset['rewards'])
